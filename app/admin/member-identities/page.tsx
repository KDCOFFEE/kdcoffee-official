...
export default async function AdminMemberIdentitiesPage() {
  ...
  const stats = [
    ...
    ["Email 登入", summary.emailIdentityCount], ["LINE 登入", summary.lineIdentityCount], ["手機密碼登入", summary.phoneIdentityCount], ["同時連結 Email + LINE", summary.bothLinkedCount],
  ...
  return <main className="admin-page"><div className="admin-back"><Link href="/admin">← 返回營運中心</Link></div><section className="admin-panel member-identity-admin">
    ...
    <p className="member-identity-admin-intro">每位會員保有一個固定會員編號，Email、LINE 與手機密碼是分開管理的登入方式。系統不會因為聯絡資料相同就自動合併會員。</p>
