...
export default async function AdminPage() {
  ...
  return <main className="admin-page"><header className="admin-topbar"><div><p className="eyebrow dark">KD COFFEE STUDIO</p><h1>工作室營運中心</h1></div><form action="/api/admin/logout" method="post"><button className="logout-button">登出後台</button></form></header><section className="admin-quick-links"><Link className="admin-logo-entry" href="/admin/logo"><strong>上傳與管理 Logo</strong><span>Header、Footer、方形標誌與分享圖 →</span></Link><Link href="/admin/homepage"><strong>首頁內容與照片</strong><span>Hero、多活動、推薦作品圖片 →</span></Link><Link href="/admin/assets"><strong>品牌資產與 Logo</strong><span>圖片編號、尺寸、用途與 SEO 自動命名 →</span></Link><Link href="/admin/products"><strong>作品與本月豆單</strong><span>23 款作品、上架、售完與首頁顯示 →</span></Link><Link href="/admin/orders"><strong>訂單管理</strong><span>查看與更新所有訂單 →</span></Link></section><section className="admin-stats"><article><small>今日訂單</small><strong>{todayOrders.length}</strong></article><article><small>待處理</small><strong>{pending.length}</strong></article><article><small>本月訂單</small><strong>{monthly.length}</strong></article><article><small>本月訂單金額</small><strong>NT$ {revenue.toLocaleString("zh-TW")}</strong></article></section><section className="admin-panel"><div className="admin-panel-head"><div><p className="eyebrow dark">LATEST ORDERS</p><h2>最新訂單</h2></div><Link href="/admin/orders">查看全部</Link></div>{orders.slice(0,10).map(order=><Link href={`/admin/orders/${order.orderNumber}`} className="admin-order-row" key={order.orderNumber}><div><strong>{order.orderNumber}</strong><span>{order.customer?.name || "未填姓名"}・{order.orderMode === "711_cod" ? order.store?.name || "7-ELEVEN" : "工作室自取"}</span></div><div><span className="order-status-chip">{orderStatusLabel(order.status)}</span><b>NT$ {Number(order.total||order.subtotal||0).toLocaleString("zh-TW")}</b></div></Link>)}{!orders.length && <p className="admin-empty">目前還沒有訂單。</p>}</section></main>;
}
