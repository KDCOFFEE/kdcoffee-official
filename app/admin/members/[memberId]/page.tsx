...
const accountLabels = { active: "正常", disabled: "已停用", "possible-duplicate": "待人工確認", "merged-tombstone": "已合併保留" } as const;
const loginMethodLabels = { email: "Email", line: "LINE", phone: "手機" } as const;
...
export default async function AdminMemberDetailPage({ params }: { params: Promise<{ memberId: string }> }) {
  ...
  const canResetMemberPassword = !adminSession.legacy && adminSession.role === "owner";
  ...
  return <main className="admin-page admin-member-detail-page">
    ...
    <header className="member-detail-header"><div><p className="eyebrow dark">MEMBER PROFILE</p><h1>{summary.name}</h1><p>{summary.memberNumber || "尚無會員編號"}・{accountLabels[summary.accountStatus]}・{summary.loginMethods.length ? summary.loginMethods.map((item) => loginMethodLabels[item]).join(" + ") : "登入方式未建立"}</p></div><div className="member-detail-credit"><small>可用抵用金</small><strong>NT$ {summary.availableCredit.toLocaleString("zh-TW")}</strong></div></header>
    ...
    <section className="admin-panel member-detail-section" id="subscriptions"><div className="admin-panel-head"><div><p className="eyebrow dark">SUBSCRIPTION</p><h2>定期購</h2></div><Link href="/admin/membership">前往會員與定期購設定 →</Link></div>{detail.subscriptions.map((subscription) => <article className="member-subscription-card" key={subscription.subscriptionId}><div><strong>{subscriptionLabels[subscription.status]}</strong><span>{subscription.intervalDays} 天週期・{subscription.shippingMethod === "711_cod" ? subscription.storeSelection?.storeName || "7-ELEVEN" : subscription.shippingMethod === "home_delivery" ? "宅配" : "工作室自取"}</span><small>從訂單 {subscription.startedFromOrderId} 建立</small></div><div><span>{subscription.defaultItems.map((item) => subscriptionItemProductIds(item).join(" + ")).join("、") || "商品資料未提供"}</span><b>{subscription.defaultItems.reduce((sum, item) => sum + item.quantity, 0)} 份</b></div>{subscription.cycles[0] ? <div><span>最近一期 {subscription.cycles[0].plannedDate}</span><b>{cycleLabels[subscription.cycles[0].status] || subscription.cycles[0].status}</b><small>修改截止 {subscription.cycles[0].modificationDeadline}</small></div> : <div><span>尚無配送期次</span></div>}</article>)}{!detail.subscriptions.length ? <p className="member-detail-empty">這位會員目前沒有定期購。</p> : null}</section>
    ...
    <section className="admin-panel member-detail-section" id="identity"><div className="admin-panel-head"><div><p className="eyebrow dark">IDENTITY</p><h2>身份與登入</h2></div><Link href="/admin/member-identities">前往會員身份系統 →</Link></div><div className="member-identity-safe-grid"><article><small>Email 登入</small><strong>{summary.loginMethods.includes("email") ? "已連結" : "未連結"}</strong><span>{detail.contact.email || "—"}</span></article><article><small>LINE 登入</small><strong>{summary.loginMethods.includes("line") ? "已連結" : "未連結"}</strong><span>不顯示任何登入憑證或權杖</span></article><article><small>手機密碼登入</small><strong>{summary.loginMethods.includes("phone") ? "已建立" : "未建立"}</strong><span>{detail.contact.phone || "—"}</span></article><article><small>帳號狀態</small><strong>{accountLabels[summary.accountStatus]}</strong></article><article><small>最近登入</small><strong>{dateTime(detail.contact.lastLoginAt)}</strong></article></div>{detail.identities.map((identity) => <p className="member-identity-row" key={identity.provider}><b>{loginMethodLabels[identity.provider]}</b><span>{identity.verifiedAt ? `驗證 ${dateTime(identity.verifiedAt)}` : "未宣告手機所有權驗證"}・連結 {dateTime(identity.linkedAt)}</span></p>)}<div className="admin-detail-section"><h3>重新設定會員密碼</h3>{!summary.loginMethods.includes("email") ? <p>這位會員目前沒有可由此工具重新設定的 Email 密碼；手機密碼復原流程尚未在本階段提供。</p> : !canResetMemberPassword ? <p>只有 Owner 可以重新設定會員的 Email 登入密碼。</p> : <><p>直接替會員設定新的 Email 登入密碼。完成後，之後使用 Email 密碼登入時必須使用新密碼，尚未使用的忘記密碼連結也會立即失效。</p><MemberPasswordResetForm memberId={summary.memberId} /></>}</div></section>
  ...
  <MemberDeletionPanel memberId={memberId} memberLabel={`${summary.name}（${summary.memberNumber || "尚無會員編號"}）`} memberStatus={summary.accountStatus} />
    </main>;
}
