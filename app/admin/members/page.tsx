...
export default async function AdminMembersPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const filters: AdminMemberListFilters = {
    ...
    login: (["email", "line", "phone", "both"].includes(value(search, "login") || "") ? value(search, "login") : "all") as AdminMemberListFilters["login"],
    ...
    status: value(search, "status") === "disabled" ? "disabled" : value(search, "status") === "possible-duplicate" ? "possible-duplicate" : value(search, "status") === "active" ? "active" : "all",
  };
  ...
  return <main className="admin-page admin-members-page">
    ...
    <section className="admin-panel member-admin-list-panel">
      <form className="member-admin-filters" method="get"><label className="member-admin-search">搜尋會員<input name="q" defaultValue={filters.query} placeholder="姓名、會員編號、Email 或手機" /></label><label>排序<select name="sort" defaultValue={filters.sort}><option value="newest">最新加入</option><option value="oldest">最早加入</option></select></label><label>登入方式<select name="login" defaultValue={filters.login}><option value="all">全部</option><option value="email">Email</option><option value="line">LINE</option><option value="phone">手機</option><option value="both">Email + LINE</option></select></label><label>定期購<select name="subscription" defaultValue={filters.subscription}><option value="all">全部</option><option value="active">進行中</option><option value="inactive">未進行</option></select></label><label>抵用金<select name="credit" defaultValue={filters.credit}><option value="all">全部</option><option value="available">有可用抵用金</option></select></label><label>推薦<select name="referral" defaultValue={filters.referral}><option value="all">全部</option><option value="participating">有參與推薦</option></select></label><label>帳號狀態<select name="status" defaultValue={filters.status}><option value="all">全部</option><option value="active">正常</option><option value="disabled">已停用</option><option value="possible-duplicate">待人工確認</option></select></label><div className="member-admin-filter-actions"><button className="admin-primary-button">套用</button><Link href="/admin/members">清除</Link></div></form>
      ...
      <div className="member-admin-list">{result.rows.map((row) => <Link className="member-admin-row" href={`/admin/members/${encodeURIComponent(row.memberId)}`} key={row.memberId}>
        <div className="member-admin-person"><strong>{row.name}</strong><span>{row.memberNumber || "尚無會員編號"}</span><small>{row.email || row.phone || "尚無聯絡資料"}</small><div className="member-chip-row">{row.loginMethods.length ? row.loginMethods.map((method) => <b key={method}>{loginMethodLabels[method]}</b>) : <b>登入方式未建立</b>}</div></div>
