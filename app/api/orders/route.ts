...
export async function POST(request: Request) {
  try {
    ...
    const activeRetailPromotionAttribution = !member && orderMode !== "corporate_gift"
      ? await readRetailPromotionOrderSnapshot({ rulesVersion })
      : null;
    ...
    const core = await withOrderIdempotencyLock(orderDir(), idempotencyKey, async () => {
      ...
      const retailPromotionAttribution: RetailPromotionOrderSnapshot | null = member
        ? null
        : existing.action === "retry" && existing.order.retailPromotionAttribution
          ? existing.order.retailPromotionAttribution as RetailPromotionOrderSnapshot
          : activeRetailPromotionAttribution;
    ...
    if (core.kind === "replay") {
      const storedMember = core.order.member && typeof core.order.member === "object" ? core.order.member as Record<string, unknown> : null;
      const storedMemberId = typeof storedMember?.memberId === "string" ? storedMember.memberId : null;
      const replayAsSameMember = Boolean(member && storedMemberId === member.id);
      ...
      if (replayAsSameMember && member && requestedCredit > 0 && core.order.orderMode !== "corporate_gift" && !(core.order.credit && typeof core.order.credit === "object" && typeof (core.order.credit as Record<string, unknown>).reservationId === "string")) {
        try {
