...
export async function GET() {
  ...
  for (const cycle of cycles) {
    ...
    rows.push([cycle.plannedDate, cycle.createdOrderId || "尚未建立", member?.displayName || member?.pickupName || "KD Coffee 會員", cycle.itemsDraft.flatMap(subscriptionItemProductIds).join(" + "), cycle.itemsDraft.reduce((sum, item) => sum + item.quantity, 0), cycle.giftSnapshot?.eligible ? cycle.giftSnapshot.quantity : 0, subscription?.shippingMethod === "711_cod" ? subscription.storeSelection?.storeName || "7-ELEVEN" : "工作室自取"].map(csv).join(","));
  }
