...
export type FavoriteStore = { id: string; name: string; address: string; city?: string; district?: string };
export type Member = {
  id: string;
  lineUserId: string;
  displayName: string;
...
export function createSessionToken(memberId: string) {
  const payload = Buffer.from(JSON.stringify({ v: 1, memberId, exp: Date.now() + 1000 * 60 * 60 * 24 * 30 })).toString("base64url");
...
export function verifySessionToken(token?: string | null) {
  ...
  try {
    ...
    if (parsed.v !== 1 || !parsed.memberId || parsed.exp < Date.now()) return null;
...
export async function clearMemberSession() {
  const jar = await cookies();
  jar.set(MEMBER_SESSION_COOKIE, "", { httpOnly: true, sameSite: "lax", secure: process.env.NODE_ENV === "production", expires: new Date(0), path: "/" });
}
export async function readMember(id: string): Promise<Member | null> {
  try { return JSON.parse(await fs.readFile(path.join(membersDir, `${id}.json`), "utf8")); } catch { return null; }
}
export async function getCurrentMember(): Promise<Member | null> {
  const jar = await cookies();
  const id = verifySessionToken(jar.get(MEMBER_SESSION_COOKIE)?.value);
  return id ? readMember(id) : null;
}
export async function saveMember(member: Member) {
  await fs.mkdir(membersDir, { recursive: true });
  await fs.writeFile(path.join(membersDir, `${member.id}.json`), JSON.stringify(member, null, 2), "utf8");
  return member;
}
export async function upsertLineMember(profile: { sub: string; name?: string; picture?: string; email?: string }) {
  await fs.mkdir(membersDir, { recursive: true });
  const id = createHmac("sha256", secret()).update(profile.sub).digest("hex").slice(0, 24);
  const existing = await readMember(id);
  const now = new Date().toISOString();
  return saveMember({
    id,
    lineUserId: profile.sub,
    displayName: profile.name || existing?.displayName || "LINE 會員",
...
export async function updateMemberProfile(memberId: string, patch: { pickupName?: string; phone?: string; email?: string; favoriteStore?: FavoriteStore }) {
...
export function randomState() { return randomBytes(24).toString("base64url"); }
