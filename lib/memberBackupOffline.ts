...
<script>const DATA=${embedded};const byId=new Map(DATA.nodes.map(function(n){return[n.memberId,n]}));const members=new Map(DATA.members.map(function(m){return[m.memberId,m]}));const esc=function(s){return String(s==null?"":s).replace(/[&<>"']/g,function(c){return{"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"}[c]})};const show=function(v){return v===null||v===undefined||v===""?"—":String(v)};const money=function(v){return typeof v==="number"?"NT$"+v.toLocaleString("zh-TW"):"—"};function branch(id,seen){seen=seen||new Set();const n=byId.get(id),m=members.get(id);if(!n||seen.has(id))return"";const next=new Set(seen);next.add(id);const kids=n.childrenIds.filter(function(x){return byId.has(x)&&!next.has(x)});return '<li data-id="'+esc(id)+'"><article class="node '+(n.depth===0?'root':'')+'" tabindex="0" role="button"><strong>'+esc((m&&m.displayName)||('會員 '+(n.memberNumber||'未編號')))+'</strong><small>'+esc(n.memberNumber||'未編號')+' · '+esc(n.memberId)+'</small><small>直推 '+n.directReferralCount+' 人 · 團隊 '+n.teamCount+' 人</small><em>'+esc(n.relationshipStatus)+'</em>'+(kids.length?'<button class="branch-toggle" type="button">收合分支</button>':'')+'</article>'+(kids.length?'<ul>'+kids.map(function(x){return branch(x,next)}).join('')+'</ul>':'')+'</li>'}const forest=document.getElementById('forest');forest.innerHTML=DATA.roots.length?DATA.roots.map(function(id){return'<ul class="tree">'+branch(id)+'</ul>'}).join(''):'<p class="empty">沒有可顯示的 root；請查看 organization-tree.json 的 validation findings。</p>';let selectedId=null;function ancestors(id){const result=[];let current=id;const seen=new Set();while(current&&!seen.has(current)){seen.add(current);result.push(current);current=byId.get(current)?.parentId||null}return result}function focusNode(id){ancestors(id).forEach(function(current){const li=document.querySelector('li[data-id="'+CSS.escape(current)+'"]');if(li)li.classList.remove('collapsed')});document.querySelectorAll('.node.match,.node.selected').forEach(function(node){node.classList.remove('match','selected')});const li=document.querySelector('li[data-id="'+CSS.escape(id)+'"]');if(!li)return;const card=li.querySelector(':scope > .node');card.classList.add('match','selected');selectedId=id;card.scrollIntoView({behavior:'smooth',block:'center',inline:'center'})}function copyRow(label,value){return'<dt>'+esc(label)+'</dt><dd class="copy-row"><span>'+esc(show(value))+'</span>'+(value?'<button type="button" data-copy="'+esc(value)+'">複製</button>':'')+'</dd>'}function countRows(value){const rows=Object.entries(value||{});return rows.length?rows.map(function(row){return esc(row[0])+' '+row[1]}).join('、'):'—'}function showProfile(id){const m=members.get(id);if(!m)return;focusNode(id);const parent=m.organization.parentId?(esc(m.organization.parentDisplayName||m.organization.parentMemberNumber||m.organization.parentId)+'｜'+esc(m.organization.parentMemberNumber||m.organization.parentId)):'—';const children=m.organization.children.length?'<ul>'+m.organization.children.map(function(child){return'<li><button type="button" data-member="'+esc(child.memberId)+'">'+esc(child.displayName||child.memberNumber||child.memberId)+'｜'+esc(child.memberNumber||child.memberId)+'</button></li>'}).join('')+'</ul>':'<span class="muted">無</span>';const subscriptions=m.subscriptions.length?'<div class="cards">'+m.subscriptions.map(function(s){const cycle=s.nextCycle;return'<div class="card"><strong>'+esc(s.subscriptionId)+' · '+esc(s.status)+'</strong><small>週期：'+(s.intervalDays==null?'—':'每 '+s.intervalDays+' 天')+'｜取貨：'+esc(show(s.shippingMethod))+(s.storeSelection?'・'+esc(s.storeSelection.storeName):'')+'</small><small>anchor：'+esc(show(s.anchorDate))+'</small><small>下一期：'+(cycle?esc(cycle.plannedDate+' · '+cycle.status+' · 截止 '+cycle.modificationDeadline+(cycle.estimatedAmount==null?'':' · '+money(cycle.estimatedAmount))):'尚無可用期次')+'</small></div>'}).join('')+'</div>':'<span class="muted">無安全關聯資料</span>';const orders=m.orders.recent.length?'<div class="cards">'+m.orders.recent.map(function(o){return'<div class="card"><strong>'+esc(o.orderNumber)+' · '+esc(show(o.status))+'</strong><small>'+esc(show(o.createdAt))+'｜'+money(o.amount)+'｜'+esc(show(o.orderMode))+(o.store?'・'+esc(o.store.name):'')+'</small><small>'+esc(o.items.join('、')||'無商品摘要')+'</small></div>'}).join('')+'</div>':'<span class="muted">無安全關聯訂單</span>';const fulfillment=m.fulfillment.length?'<div class="cards">'+m.fulfillment.map(function(f){const last=f.events[f.events.length-1];return'<div class="card"><strong>'+esc(f.orderNumber)+' · '+esc(show(f.currentState))+'</strong><small>更新：'+esc(show(f.updatedAt))+(last?'｜最近事件 '+esc(last.state)+' '+esc(last.occurredAt):'')+'</small></div>'}).join('')+'</div>':'<span class="muted">無安全關聯履約資料</span>';const profile=document.getElementById('profile');profile.innerHTML='<div class="profile-head"><div><span class="badge">離線備份資料｜含會員個資</span><h2 id="profile-title">'+esc(m.displayName||m.memberNumber||m.memberId)+'</h2><p>'+esc(m.memberNumber||'未編號')+' · '+esc(m.memberId)+'</p></div><button class="profile-close" type="button" aria-label="關閉">×</button></div><section><h3>IDENTITY</h3><dl>'+copyRow('姓名',m.displayName)+copyRow('會員編號',m.memberNumber)+copyRow('Member ID',m.memberId)+copyRow('手機',m.phone)+copyRow('Email',m.email)+copyRow('登入 Email',m.loginEmail)+'<dt>建立日期</dt><dd>'+esc(show(m.identity.createdAt))+'</dd><dt>帳號狀態</dt><dd>'+esc(show(m.identity.status))+'</dd><dt>Identity Provider</dt><dd>'+esc(m.identity.providerTypes.join('、')||'—')+'</dd><dt>自訂頭像</dt><dd>'+(m.identity.customAvatar?'有':'無')+'</dd><dt>Provider 圖片</dt><dd>'+(m.identity.providerPicture?'有':'無')+'</dd></dl></section><section><h3>ORGANIZATION</h3><dl><dt>上層推薦人</dt><dd>'+parent+'</dd><dt>直推／團隊／後代</dt><dd>'+m.organization.directReferralCount+'／'+m.organization.teamCount+'／'+m.organization.descendantCount+'</dd><dt>深度</dt><dd>'+esc(show(m.organization.depth))+'</dd><dt>關係狀態</dt><dd>'+esc(show(m.organization.relationshipStatus))+'</dd><dt>直推名單</dt><dd>'+children+'</dd></dl></section><section><h3>SUBSCRIPTION</h3>'+subscriptions+'</section><section><h3>COMMERCE / RIGHTS</h3><dl><dt>資料中抵用金餘額合計</dt><dd>'+money(m.commerce.recordedRemainingCredit)+'</dd><dt>Credit entries</dt><dd>'+m.commerce.creditEntriesCount+'（'+countRows(m.commerce.creditStatusCounts)+'）</dd><dt>Referral rewards</dt><dd>'+m.commerce.rewardCount+'（'+countRows(m.commerce.rewardStatusCounts)+'）</dd><dt>回饋金額</dt><dd>已計算 '+money(m.commerce.calculatedRewardAmount)+'／Projected '+money(m.commerce.projectedRewardAmount)+'</dd><dt>Effective PV</dt><dd>'+m.commerce.effectivePV+'</dd><dt>有效消費</dt><dd>'+m.commerce.validConsumptionCount+' 筆／'+money(m.commerce.validConsumptionAmount)+'</dd><dt>資格輪次</dt><dd>'+m.commerce.qualificationRoundCount+'</dd></dl></section><section><h3>ORDERS（'+m.orders.totalCount+'）</h3>'+orders+'</section><section><h3>FULFILLMENT</h3>'+fulfillment+'</section><div class="profile-actions"><button type="button" data-back>回到組織圖</button><button type="button" class="profile-close">關閉</button></div>';document.getElementById('shade').hidden=false;profile.hidden=false;profile.querySelector('.profile-close').focus()}function closeProfile(){document.getElementById('shade').hidden=true;document.getElementById('profile').hidden=true;if(selectedId){const card=document.querySelector('li[data-id="'+CSS.escape(selectedId)+'"] > .node');if(card)card.focus()}}forest.addEventListener('click',function(e){const toggle=e.target.closest('.branch-toggle');if(toggle){e.stopPropagation();const li=toggle.closest('li');li.classList.toggle('collapsed');toggle.textContent=li.classList.contains('collapsed')?'展開分支':'收合分支';return}const card=e.target.closest('.node');if(card)showProfile(card.closest('li').dataset.id)});forest.addEventListener('keydown',function(e){if((e.key==='Enter'||e.key===' ')&&e.target.classList.contains('node')){e.preventDefault();showProfile(e.target.closest('li').dataset.id)}});let scale=1,ox=0,oy=0;const stage=document.getElementById('stage'),vp=document.getElementById('viewport'),out=document.getElementById('zoom');function paint(){stage.style.transform='translate('+ox+'px,'+oy+'px) scale('+scale+')';out.textContent=Math.round(scale*100)+'%'}function setScale(v){scale=Math.max(.25,Math.min(2.5,v));paint()}document.getElementById('minus').onclick=function(){setScale(scale-.1)};document.getElementById('plus').onclick=function(){setScale(scale+.1)};function reset(){scale=1;ox=0;oy=0;vp.scrollTo(0,0);paint()}document.getElementById('reset').onclick=reset;document.getElementById('expand').onclick=function(){document.querySelectorAll('li.collapsed').forEach(function(x){x.classList.remove('collapsed')});document.querySelectorAll('.branch-toggle').forEach(function(x){x.textContent='收合分支'})};let drag=null;vp.addEventListener('pointerdown',function(e){if(e.target.closest('button,input'))return;drag={x:e.clientX,y:e.clientY,ox:ox,oy:oy};vp.setPointerCapture(e.pointerId);vp.classList.add('dragging')});vp.addEventListener('pointermove',function(e){if(!drag)return;ox=drag.ox+e.clientX-drag.x;oy=drag.oy+e.clientY-drag.y;paint()});function stop(){drag=null;vp.classList.remove('dragging')}vp.addEventListener('pointerup',stop);vp.addEventListener('pointercancel',stop);vp.addEventListener('wheel',function(e){if(!e.ctrlKey)return;e.preventDefault();setScale(scale+(e.deltaY<0?.08:-.08))},{passive:false});function scoreMember(m,q){const textQ=q.trim().toLocaleLowerCase('zh-Hant'),phone=q.trim().replace(/[\s\-()]/g,''),email=q.trim().toLowerCase();const fields=m.search;if(!textQ)return null;let score=0,exact=false;if(fields.memberNumber===textQ){score=130;exact=true}else if(fields.memberId===textQ){score=125;exact=true}else if(phone&&fields.phoneNormalized===phone){score=120;exact=true}else if(email&&(fields.emailNormalized===email||fields.loginEmailNormalized===email)){score=115;exact=true}else if(fields.displayName===textQ){score=110;exact=true}else if(fields.displayName.includes(textQ))score=80;else if(fields.memberNumber.includes(textQ))score=75;else if(fields.memberId.includes(textQ))score=70;else if(textQ.length>=3&&(fields.emailNormalized.includes(email)||fields.loginEmailNormalized.includes(email)))score=65;else if(phone.length>=4&&fields.phoneNormalized.includes(phone))score=60;return score?{m:m,score:score,exact:exact}:null}const results=document.getElementById('results');function renderResults(matches){results.hidden=false;if(!matches.length){results.innerHTML='<p class="result-empty">找不到符合的會員</p>';return}results.innerHTML=matches.map(function(r){const m=r.m;return'<button class="result" type="button" data-member="'+esc(m.memberId)+'"><strong>'+esc(m.displayName||'未提供姓名')+'｜'+esc(m.memberNumber||'未編號')+'</strong><small>'+esc(m.maskedPhone||'無手機')+'｜'+esc(m.maskedEmail||'無 Email')+'｜直推 '+m.organization.directReferralCount+'／團隊 '+m.organization.teamCount+'</small></button>'}).join('')}function find(){const q=document.getElementById('search').value;const matches=DATA.members.map(function(m){return scoreMember(m,q)}).filter(Boolean).sort(function(a,b){return b.score-a.score||a.m.memberNumber.localeCompare(b.m.memberNumber)||a.m.memberId.localeCompare(b.m.memberId)});const exact=matches.filter(function(r){return r.exact});if(exact.length===1){results.hidden=true;showProfile(exact[0].m.memberId);return}renderResults(matches)}document.getElementById('find').onclick=find;document.getElementById('search').addEventListener('input',function(e){if(!e.target.value.trim())results.hidden=true;else find()});document.getElementById('search').addEventListener('keydown',function(e){if(e.key==='Enter')find();if(e.key==='Escape')results.hidden=true});results.addEventListener('click',function(e){const button=e.target.closest('[data-member]');if(button){results.hidden=true;showProfile(button.dataset.member)}});document.getElementById('profile').addEventListener('click',function(e){const member=e.target.closest('[data-member]');if(member){showProfile(member.dataset.member);return}const copy=e.target.closest('[data-copy]');if(copy){const value=copy.dataset.copy;const fallback=function(){const area=document.createElement('textarea');area.value=value;document.body.appendChild(area);area.select();document.execCommand('copy');area.remove()};if(navigator.clipboard&&navigator.clipboard.writeText)navigator.clipboard.writeText(value).catch(fallback);else fallback();copy.textContent='已複製';return}if(e.target.closest('.profile-close'))closeProfile();if(e.target.closest('[data-back]')){closeProfile();if(selectedId)focusNode(selectedId)}});document.getElementById('shade').onclick=closeProfile;document.addEventListener('keydown',function(e){if(e.key==='Escape'){results.hidden=true;if(!document.getElementById('profile').hidden)closeProfile()}});paint();</script></body></html>`;
}
