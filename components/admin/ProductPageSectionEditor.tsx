...
import CleanRoastingMediaAdmin from "@/components/admin/CleanRoastingMediaAdmin";
import {
...
type Props = { selected: Product; products: Product[]; patch: (change: Record<string, unknown>) => void; setMessage: (message: string) => void };
...
export default function ProductPageSectionEditor({ selected, products, patch, setMessage }: Props) {
  ...
  return <section className="cms-panel product-section-layout-editor">
    ...
    <CustomProductSectionManager key={String(selected.id || selected.slug || "product")} selected={selected} patch={patch} />
