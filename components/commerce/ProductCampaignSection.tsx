...
export default function ProductCampaignSection({ campaigns }: { campaigns: HomepageCampaign[] }) {
  const presentationRef = useRef<HTMLDivElement>(null);
  const [activeIndex, setActiveIndex] = useState(0);

  useEffect(() => {
    presentationRef.current?.classList.add("is-product-campaign-enhanced");
  }, []);

  if (!campaigns.length) return null;

  return (
    <section className="revenue-content-section product-campaign-section" aria-labelledby="product-campaign-title">
      ...
      <div className="product-campaign-presentation" ref={presentationRef}>
        <div className="product-campaign-stack">
          {campaigns.map((campaign, index) => (
            <article className={`product-campaign-card${index === activeIndex ? " is-active" : ""}`} key={campaign.id}>
        ...
        {campaigns.length > 1 ? (
          <div className="product-campaign-controls" aria-label="切換最新活動">
            {campaigns.map((campaign, index) => (
              <button type="button" key={campaign.id} aria-pressed={index === activeIndex} onClick={() => setActiveIndex(index)}>
